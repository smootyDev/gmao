...
@Component
public class AnthropicAdapter implements AiProvider {
    ...
    public AiCompletion complete(String systemPrompt, String userPrompt, AiCallSettings settings) {
        ...
        body.put("max_tokens", settings.maxTokens() == null ? 1000 : settings.maxTokens());
    ...
    private String buildEndpoint(AiCallSettings settings) {
        ...
        if (baseUrl == null || baseUrl.isBlank()) {
            throw new AiProviderException("Falta la URL base del proveedor anthropic");
        }
    ...
    private AiCompletion parse(JsonNode response) {
        ...
        if (content.isArray()) {
            for (JsonNode block : content) {
                if ("text".equals(block.path("type").asText(""))) {
                    String chunk = block.path("text").asText("");
                    if (!chunk.isBlank()) {
                        if (!text.isEmpty()) {
                            text.append("\n");
                        }
        ...
        long tokensIn = response.path("usage").path("input_tokens").asLong(0);
        long tokensOut = response.path("usage").path("output_tokens").asLong(0);
